...
Chat được bật bằng migration `supabase/chat_messages.sql`. Migration này tạo conversations cho chat riêng/phòng, thành viên, tin nhắn có `expires_at` sau 60 phút và bảng reports. Chạy migration sau `user_profiles.sql`, `social_friends.sql` và `listening_rooms.sql`. Sau đó chạy thêm `supabase/chat_attachments.sql` để thêm metadata ảnh/tệp vào `chat_messages`.

Frontend ưu tiên WebSocket tại `/api/chat/ws` để nhận tin nhắn và attachment nhanh trong cùng instance. Khi WebSocket không kết nối được hoặc request đi qua instance khác, Chat Hub tự chuyển sang fallback polling để nội dung vẫn được cập nhật. Backend luôn lọc `expires_at` khi đọc; cleanup định kỳ xóa row hết hạn và xóa asset attachment tương ứng trên Cloudinary trước. Admin có thể gọi cleanup thủ công từ API khi cần.

Chat không dùng player, audio element hoặc queue cá nhân. Người dùng có thể gửi JPG/PNG/WebP/GIF, PDF, TXT, CSV, JSON, ZIP và một số file Office; giới hạn hiện tại là 25 MiB mỗi attachment, được upload server-side lên Cloudinary và không đưa secret ra frontend. Tin nhắn cùng ảnh/tệp bị xóa khỏi database hoạt động sau 60 phút; không nên mô tả là xóa được screenshot, cache thiết bị, backup hoặc log hạ tầng. Backend có rate limit nhẹ 30 tin nhắn hoặc attachment mỗi user trong 60 giây trên từng Render instance; khi mở rộng nhiều instance cần Redis hoặc Supabase RPC để có limiter phân tán. WebSocket registry cũng nằm trong RAM từng instance, vì vậy UI luôn giữ polling fallback 4 giây và không quảng cáo zero delay hoặc realtime xuyên instance tuyệt đối.

Sau khi chạy đủ `chat_messages.sql` và `chat_attachments.sql` rồi để Render/Vercel deploy, user chỉ có thể mở direct chat với bạn bè đã accepted và không bị block. Backend vẫn có fallback tương thích để đọc và xóa chat chữ khi `chat_attachments.sql` chưa chạy; chỉ luồng gửi attachment mới yêu cầu migration này. Room member có thể mở chat phòng từ Listening Room. Kiểm tra bằng hai tài khoản: gửi message, ảnh và PDF ở một trình duyệt, xác nhận trình duyệt kia nhận realtime và mở được attachment; tắt WebSocket hoặc đổi mạng để kiểm tra fallback polling. Nếu chưa chạy `chat_attachments.sql`, chat chữ vẫn có thể hoạt động nhưng đọc/gửi attachment sẽ báo migration chưa được bật.


## AI Music Concierge

AI Concierge chạy qua endpoint server-side `POST /api/ai/concierge`; frontend không nhận hoặc lưu Gemini key. Backend đọc một trong hai cấu hình sau trên Render: `GEMINI_API_KEYS` là danh sách key phân cách bằng dấu phẩy, hoặc các slot `GEMINI_API_KEY_1` đến `GEMINI_API_KEY_10`. Có thể đặt `GEMINI_MODEL` (mặc định `gemini-3.7-flash`) và `LUNU_AI_RATE_LIMIT_COUNT` (mặc định 12 request/phút/người dùng). File mẫu nằm tại `backend/.env.example`.

Gemini output được yêu cầu ở dạng JSON gồm câu trả lời, mood và danh sách `song_id`; backend lọc lại toàn bộ recommendation theo catalog Supabase trước khi trả về frontend. Vì vậy AI không thể tạo ra URL audio hoặc bài hát ngoài thư viện để frontend phát tùy ý. Nút **AI Concierge** mở panel chat, mỗi recommendation có **Phát ngay** và **Thêm vào hàng đợi**. Lưu ý quota Gemini được tính theo **project** với các chiều RPM, TPM và RPD, không nhân lên theo số API key; 10 slot chỉ giúp fallback khi một credential lỗi hoặc hết khả dụng, không tạo thêm 10 lần quota.

Không commit key thật vào GitHub, `.env` hoặc Vercel. Google khuyến nghị giữ key trong environment/backend và không expose key phía client; tài liệu API hiện tại cũng hỗ trợ structured output cho dữ liệu có schema: [Gemini API keys](https://ai.google.dev/gemini-api/docs/api-key) và [structured outputs](https://ai.google.dev/gemini-api/docs/structured-output). Sau khi thêm secrets trên Render, redeploy backend rồi kiểm tra đăng nhập, mở **AI Concierge**, gửi thử `Nay t buồn, gợi ý nhạc thất tình` và xác nhận nút phát/queue hoạt động.
