...
| `LUNU_AUDIO_UPLOAD_MAX_BYTES` | Optional direct audio upload limit; defaults to 200 MiB. |
...
When Render receives YouTube `429`/`403` responses, the downloader now stops immediately instead of retrying every player client. In **Admin → Kho nhạc**, use **Upload audio trực tiếp** to upload an audio file that you are authorized to store; the backend normalizes it with FFmpeg and sends it to Cloudinary without depending on YouTube.
