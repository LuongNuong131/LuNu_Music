...
const cleanupOld = async () => { const value = window.prompt('Dọn các thông báo đã đọc cũ hơn bao nhiêu ngày trên toàn hệ thống?', '30'); if (value === null) return; const days = Number(value); if (!Number.isInteger(days) || days < 1) { actionMessage.value = 'Số ngày không hợp lệ.'; return; } try { const response = await cleanupNotifications(days); actionMessage.value = response.message || `Đã dọn ${response.deleted_count || 0} thông báo.`; await sync(); } catch (error) { actionMessage.value = error.message || 'Không thể dọn Inbox hệ thống.'; } };
