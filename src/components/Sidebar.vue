<template>
  <aside class="glass-sidebar">
    ...
    <div class="user-info"><div class="avatar">{{ (authState.user?.username || 'U').slice(0, 1).toUpperCase() }}</div><div class="user-copy"><strong>{{ authState.user?.username || 'Listener' }}</strong><span>{{ authState.user?.role === 'admin' ? 'Administrator' : 'Member' }}</span></div><button @click="logoutUser" class="logout-btn" aria-label="Đăng xuất">↗</button></div>
  </aside>
</template>
