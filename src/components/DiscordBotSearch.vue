<template>
  <div class="glass-search-container">
    ...
    <!-- DANH SÁCH KẾT QUẢ YOUTUBE -->
    <div v-if="searchResults.length > 0" class="results-list">
      <div v-for="video in searchResults" :key="video.id" class="result-item">
        <img :src="'https://i.ytimg.com/vi/' + video.id + '/hqdefault.jpg'" alt="thumb" class="video-thumb" />
        
        <div class="video-info">
          <h4 class="video-title" :title="video.title">{{ video.title }}</h4>
          <p class="video-channel">{{ video.uploader }}</p>
        </div>
...
const handleSearch = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      message.value = `Tìm thấy ${res.results.length} kết quả. Ông chọn bài chuẩn để tải đi!`;
    } else {
      message.value = res.message || 'Không tìm thấy kết quả.';
    }
  } catch (error) {
    message.value = 'Có lỗi xảy ra khi quét YouTube!';
  } finally {
...
const handleDownload = async (video) => {
  ...
  try {
    ...
    // Tự động clear danh sách tìm kiếm và load lại nhạc sau 15s chờ BE xử lý xong
    setTimeout(() => {
      emit('song-added');
    ...
    message.value = 'Có lỗi khi tải bài này!';
    isDownloading.value = false;
    downloadingId.value = null;
  }
};
